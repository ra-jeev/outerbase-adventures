...
<div id="container">
  ...
  <button id="play-audio" class="svg-btn">
...
class OuterbasePluginCell_$PLUGIN_ID_COLUMN extends HTMLElement {
  ...
  render() {
    ...
    const audioUrlInput = this.shadow.getElementById("audio-url");
    const playAudioBtn = this.shadow.getElementById("play-audio");

    audioUrlInput.value = srcUrl;

    audioUrlInput.addEventListener("blur", () => {
      triggerEvent_$PLUGIN_ID(this, {
        action: OuterbaseColumnEvent_$PLUGIN_ID.updateCell,
        value: audioUrlInput.value,
      });
    });
